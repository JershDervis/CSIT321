...
if (empty($_POST['unitID']))
    throw new Exception('UnitID is missing');
...
if(move_uploaded_file($tmpImgName, FILES . $uniqueFileName)) {
    ...
    try {
        ...
        $stmt = $db->prepare('UPDATE unit u SET u.unit_img= :fileID WHERE u.id= :unitID ;');
        $stmt->execute(array(
            'fileID'  =>  $fileID,
            'unitID'  =>  $unitID
        ));
    ...
    $data = array(
        ...
        'log'       =>  'Uploaded file: ' . $userImgName . ', updated unit ' . $unitID . '.unit_img = ' . $fileID
    );
} else {
